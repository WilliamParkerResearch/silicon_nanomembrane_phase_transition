...
volumes_d_loop = np.linspace(vol_d1 - (vol_d2 - vol_d1) / 3, vol_d2 + (vol_d2 - vol_d1), numvol)*1e30
volumes_b_loop = np.linspace(vol_b1, vol_b2, numvol)*1e30

# while vol_d_del >= vol_d_del_0:
while vol_d_del >= vol_d_del_0:
    while pt_del >= pt_del_0:
        eos_d_loop = eos(fit_parameters_diamond,volumes_d_loop*1e-30)*1e17
        eos_d_loop_prime = np.gradient(eos_d_loop,volumes_b_loop)
        eos_b_loop = eos(fit_parameters_betasn,volumes_b_loop*1e-30)*1e17
        eos_b_loop_prime = np.gradient(eos_b_loop,volumes_b_loop)


        def tangent_beta_loop(volb_idx,vol):
            t = eos_b_loop_prime[volb_idx]*(vol-volumes_b_loop[volb_idx])+eos_b_loop[volb_idx]
            return t
        ...
        for i in range(numvol):
            ...
            intersection_equations = tangent_beta_loop(i,volumes_d_loop)-eos_d_loop
            ...
            if not np.all((intersection_points == 0)):
                ...
                vol_d_del = np.abs(vol_d2-vol_d1)
                vol_b1 = volumes_b_loop[i-1]
                vol_b2 = volumes_b_loop[i]
                vol_b_del = np.abs(vol_b2-vol_b1)
                pt1 = eos_b_loop_prime[i-1]*1e4
                pt2 = eos_b_loop_prime[i]*1e4
...
plt.plot(volumes_d_loop,1e17*eos(fit_parameters_diamond,volumes_d_loop*1e-30),color='black')
plt.show()