...
# Import correct file
if system == 'slab':
    from eos_information_slab import *
elif system == 'bulk':
    import matplotlib.pyplot as plt
    from eos_information_bulk import *
# exchange_correlation = 'SCAN'          ###CURRENTLY NEED TO CHANGE IN eos_information_slab.py or eos_information_bulk.py
write_png_file = False                 # Write PNG file if True, open plot window if False
tangent_line_shift = 0            # Shift tangent line by this factor of the vertical axis range
...
if system == 'slab':
    if exchange_correlation == 'PZ':
        ...
        diamond_bulk_modulus_horizontal_shift_factor = 4
        beta_tin_bulk_modulus_horizontal_shift_factor = 0
        transition_pressure_vertical_shift_factor = 4
...
plt.plot([beta_tin_equilibrium_volume,beta_tin_equilibrium_volume],[plot_energy_minimum-0.3*plot_energy_range,beta_tin_equilibrium_energy],color='dimgray',linestyle='--')
plt.plot([diamond_equilibrium_volume,diamond_equilibrium_volume],[plot_energy_minimum-0.3*plot_energy_range,diamond_equilibrium_energy],color='dimgray',linestyle='--')
...
if t_pres > 0:
    ...
    transition_pressure_coordinates = (beta_tin_equilibrium_volume +
                                       ...
                                       total_energies_strain_diamond[-1] -
                                       transition_pressure_vertical_shift_factor*text_vertical_shift)
    ax.text(volumes_sim_BetaSn[0], plot_energy_minimum -0.25*plot_energy_range,
             r'$V_0$ = ', color='dimgray')
    # diamond_equilibrium_volume_coordinates[1] - 0.25 * text_vertical_shift
...
if t_pres > 0:
    ax.text(diamond_equilibrium_volume_coordinates[0], diamond_equilibrium_volume_coordinates[1],
        diamond_equilibrium_volume_string, color='dimgray')
    ax.text(beta_tin_equilibrium_volume_coordinates[0], beta_tin_equilibrium_volume_coordinates[1],
            beta_tin_equilibrium_volume_string, color='dimgray')
else:
    ax.text(diamond_equilibrium_volume_coordinates[0], plot_energy_minimum-0.25*plot_energy_range,
        diamond_equilibrium_volume_string, color='dimgray')
    ax.text(beta_tin_equilibrium_volume_coordinates[0], plot_energy_minimum-0.25*plot_energy_range,
            beta_tin_equilibrium_volume_string, color='dimgray')
