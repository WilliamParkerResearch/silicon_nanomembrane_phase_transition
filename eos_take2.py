...
if exchange_correlation == 'PZ':
    ...
    volumes_sim_BetaSn = cubic_meters_per_cubic_angstrom * np.array(
        [10.23445197309, 11.69651654014, 13.15858111091, 14.62064567796, 16.08271024500, 17.54477481578,
         19.00683938282])
    total_energies_strain_BetaSn = (joules_per_rydberg / n_atom_betasn) * np.array(
        [-89.42978406, -90.39775686, -90.72865479, -90.79526219, -90.76126960, -90.69999619, -90.63787635])
    volumes_sim_diamond = cubic_meters_per_cubic_angstrom * np.array(
        [14.08392451545, 16.09591373539, 18.10790294570, 20.11989216564, 22.13188138559, 24.14387059586,
         26.15585981581])
    total_energies_strain_diamond = (joules_per_rydberg / n_atom_betasn) * np.array(
        [-90.51829730, -90.62944720, -90.70666769, -90.73095209, -90.70955754, -90.66993654, -90.60806563])
...
if exchange_correlation == 'SCAN':
    ...
    # 1ML
    volumes_sim_diamond = cubic_meters_per_cubic_angstrom * np.array(
        [14.52628826284, 16.60147230042, 18.67665633796, 20.75184037550, 22.82702441307, 24.90220845061,
         26.97739248815])
    total_energies_strain_diamond = (joules_per_rydberg / n_atom_betasn) * np.array(
        [-62.40147291, -62.52590657, -62.59658498, -62.62109441, -62.59634954, -62.54054670, -62.46430775])
    volumes_sim_BetaSn = cubic_meters_per_cubic_angstrom * np.array(
        [10.24223732525, 11.70541408762, 13.16859084621, 14.63176760859, 16.09494437096, 17.55812112953,
         19.02129789190])
    total_energies_strain_BetaSn = (joules_per_rydberg / n_atom_betasn) * np.array(
        [-61.23622805, -62.21374928, -62.54698918, -62.61339612, -62.57908878, -62.51453396, -62.44756775])
...
fit_parameters_diamond = sp.fmin(square_differences, initial_parameters_diamond,
                                 args=(volumes_sim_diamond, total_energies_strain_diamond, murnaghan), maxiter=100000)
...
initial_parameters_shape_BetaSn = (
    total_energies_strain_BetaSn[mid(total_energies_strain_BetaSn)], 1e11, 3.5,
    volumes_sim_BetaSn[mid(volumes_sim_BetaSn)])
fit_parameters_shape_BetaSn = sp.fmin(square_differences, initial_parameters_shape_BetaSn,
                                      args=(volumes_sim_BetaSn, total_energies_strain_BetaSn, murnaghan),
                                      maxiter=100000)
...
# H(P) = E + PV
#        E0 + (K0 V0 /(K0' - 1)) * ( (1+(K0'/K0)*P)^(K0'-1)/K0' - 1)
...
print("pt = ", t_pres)
print("diamond")
print("vol0 = ", vol_0_diamond, " volt = ", vol_t_diamond, " K0 = ", k_0_diamond, " E0/atom = ",
      fit_parameters_diamond[0] / (1.6e-19))

print("betatin")
print("vol0 = ", vol_0_betasn, " volt = ", vol_t_betasn, " K0 = ", k_0_betasn, " E0/atom = ",
      fit_parameters_shape_BetaSn[0] / (1.6e-19))
