...
if system == 'slab':
    if exchange_correlation == 'PZ':
        ...
        transition_pressure_vertical_shift_factor = 3
        transition_pressure_horizontal_shift_factor = 85
        ...
        beta_tin_bulk_modulus_horizontal_shift_factor = 0
        transition_pressure_vertical_shift_factor = 3.5
        transition_pressure_horizontal_shift_factor = 85
elif system == 'bulk':
    if exchange_correlation == 'PZ':
...
minvol=np.amin(total_volumes)/cubic_meters_per_cubic_angstrom
maxvol=np.amax(total_volumes)/cubic_meters_per_cubic_angstrom
...
if system == 'slab':
    allenergies=np.append(total_energies_strain_BetaSn,total_energies_strain_diamond)
if system == 'bulk':
    allenergies=np.append(total_energies_strain_shape_BetaSn,total_energies_strain_diamond)
minenergy=np.amin(allenergies)*energy_conversion_factor
maxenergy=np.amax(allenergies)*energy_conversion_factor
enerdiff=maxenergy-minenergy
topenergydisp=maxenergy+0.1*enerdiff
bottomenergydisp=minenergy-0.4*enerdiff
energy_range=(bottomenergydisp,topenergydisp)
# Use LaTeX
...
#   Vertical
ax.set_ylim(bottom=energy_range[0],top=energy_range[1])
...
plot_energy_minimum, plot_energy_maximum = minenergy, maxenergy
plot_energy_range = topenergydisp-bottomenergydisp
diamond_line_segment_fraction = diamond_equilibrium_energy-0.05*plot_energy_range
beta_tin_line_segment_fraction = beta_tin_equilibrium_energy-0.05*plot_energy_range
ax.plot([diamond_equilibrium_volume,diamond_equilibrium_volume],[bottomenergydisp,diamond_line_segment_fraction], color='black', linestyle='--')
ax.plot([beta_tin_equilibrium_volume,beta_tin_equilibrium_volume],[bottomenergydisp,beta_tin_line_segment_fraction], color='black', linestyle='--')
...
diamond_equilibrium_volume_coordinates = (diamond_equilibrium_volume +
                                          ...
                                          bottomenergydisp + text_vertical_shift)
...
if t_pres > 0:
    ...
    transition_pressure_coordinates = (beta_tin_equilibrium_volume +
                                       ...
                                       total_energies_strain_diamond[-1] -
                                       transition_pressure_vertical_shift_factor*text_vertical_shift)
    ax.text(tvol_beta/cubic_meters_per_cubic_angstrom-1.5, diamond_equilibrium_volume_coordinates[1],
             r'$V_0$ = ', color='black')
