...
min_energy = np.sort(total_energies_ecut_BetaSn)[0]
max_energy = np.sort(total_energies_ecut_BetaSn)[-1]
...
plt.title(r'Cutoff Energy Convergence of Total Energy for ' + structure_names[1] + ' ' + chemical_formula)
plt.xlabel(r"Cutoff Energy (eV)")
plt.ylabel("Total Energy (meV/atom)")
plt.xlim(cutoff_energies_BetaSn[0], cutoff_energies_BetaSn[-1])
