...
total_energies_strain_betasn[mid(total_energies_strain_betasn)], 1e11, 3.5, volumes_sim_betasn[mid(volumes_sim_betasn)])
...
while slopediff >= slope_precision:
    ...
    turnpoint_idx_beta_1, turnpoint_idx_beta_2 = int(np.where(np.diff(intersection_num_array) == 2)[0][0]) - 10, int(
        np.where(np.diff(intersection_num_array) == 2)[0][0]) + 10
    pos_tvol_beta1, pos_tvol_beta2 = volumes_betasn_loop[turnpoint_idx_beta_1], volumes_betasn_loop[
        turnpoint_idx_beta_2]
...
print('pt:', eosprime(fit_parameters_betasn, pos_tvol_beta1) * 1e-9, '-',
      eosprime(fit_parameters_betasn, pos_tvol_beta2) * 1e-9)
